...
class CredentialStore:
    ...
    def __init__(self, mode="system"):
        ...
        if mode == "system":
            try:
                ...
                raise WifiError("Secure OS credential storage is unavailable; unlock your keyring") from None
    ...
    def get_credential(self, identity: str):
        try:
            return self.memory.get(identity) if self.mode == "simulation" else self.backend.get_password("WiFiSense", identity)
        except Exception:
    ...
    def delete_credential(self, identity: str):
        try:
            ...
            elif self.backend.get_password("WiFiSense", identity) is not None:
